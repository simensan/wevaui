...
```sh
godot --headless --editor --path Tools/godot-ime-repro --quit-after 60
cc -shared -fPIC -O2 Tools/godot-ime-repro/trace_xim.c -ldl -o /path/to/libtrace_xim.so
python3 hosts/godot/check_ime_linux.py \
  --godot /path/to/godot \
  --project Tools/godot-ime-repro --native-line-edit \
...
```sh
git apply /path/to/unityui/Tools/godot-ime-repro/godot-x11-ime-commit.patch
...
patch -p1 < /path/to/unityui/Tools/godot-ime-repro/ibus-1.5.29-preedit.patch
make -C src -j12 libibus-1.0.la
```
