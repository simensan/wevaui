...
const CHROME = path.resolve(process.cwd(), '..', '..', 'Assets', 'UI', 'dialogue.html.chrome-layout.json');
const UNITY  = path.resolve(process.cwd(), '..', '..', 'Assets', 'UI', 'dialogue.unity-layout.json');

const chrome = JSON.parse(fs.readFileSync(CHROME, 'utf8'));
const unity  = JSON.parse(fs.readFileSync(UNITY, 'utf8'));

// Unity dump includes <html>/<body> wrappers Chrome strips. Skip those.
// Match by (tag, cls) only — depth differs between Chrome and Unity for
// inline synthesised elements (em/strong sit one level deeper in our
// LineBox-walk than in Chrome's DOM), so depth as bucket key drops them.
const unityFiltered = unity.filter(el => el.tag !== 'html' && el.tag !== 'body');
const unityBuckets = new Map();
for (const el of unityFiltered) {
    const key = `${el.tag}|${el.cls}`;
...
for (const r of mismatches.slice(0, 40)) {
    if (r.missing) { const c = r.chrome; console.log(`  MISS     ${r.tag.padEnd(10)} ${r.cls.padEnd(35)} ${formatRect(c).padEnd(28)} (missing in unity)`); continue; }
