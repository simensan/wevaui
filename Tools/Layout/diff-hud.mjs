// Diff Chrome's hud layout against Unity's, matching elements by
// (tag, cls, depth) in document order. Prints a table of mismatches
// sorted by largest delta first so we can attack the biggest visual
// discrepancies first.
import fs from 'node:fs';
import path from 'node:path';

const REPO = path.resolve(new URL('.', import.meta.url).pathname, '..', '..').replace(/^\/+/, '');
const CHROME = path.resolve(process.cwd(), '..', '..', 'Assets', 'UI', 'hud.html.chrome-layout.json');
const UNITY  = path.resolve(process.cwd(), '..', '..', 'Assets', 'UI', 'hud.unity-layout.json');

const chrome = JSON.parse(fs.readFileSync(CHROME, 'utf8'));
const unity  = JSON.parse(fs.readFileSync(UNITY, 'utf8'));

// Index Unity by (tag, cls, depth, occurrenceIndex) so duplicates pair correctly
...
for (const ce of chrome.elements) {
    ...
    if (!ue) {
        rows.push({
            tag: ce.tag, cls: ce.cls, depth: ce.depth,
            missing: 'unity',
            chrome: { x: ce.x, y: ce.y, w: ce.w, h: ce.h },
        });
        continue;
    }
    ...
    rows.push({
        tag: ce.tag, cls: ce.cls, depth: ce.depth,
        chrome: { x: ce.x, y: ce.y, w: ce.w, h: ce.h },
        unity:  { x: ue.x, y: ue.y, w: ue.w, h: ue.h },
        d: { x: dx, y: dy, w: dw, h: dh },
        maxDelta,
    });
}

const tol = 2.0; // px tolerance; anything below this is "match"
const mismatches = rows.filter(r => r.missing || (r.maxDelta || 0) > tol);
mismatches.sort((a, b) => (b.maxDelta || 0) - (a.maxDelta || 0));

console.log(`Chrome elements: ${chrome.elements.length}   Unity elements (filtered to element-backed): ${unity.length}`);
console.log(`Mismatches (>${tol}px): ${mismatches.length} / ${rows.length}`);
console.log('');
console.log('  delta    tag         cls                                  chrome (x,y,w,h)            unity (x,y,w,h)');
console.log('  -------  ----------  -----------------------------------  ---------------------------  ---------------------------');
for (const r of mismatches.slice(0, 40)) {
    ...
    console.log(`  ${max}   ${r.tag.padEnd(10)} ${r.cls.padEnd(35)} ${formatRect(c).padEnd(28)} ${formatRect(u)}    d(${d.x},${d.y},${d.w},${d.h})`);
}

function formatRect(r) {
    return `(${r.x.toFixed(0)},${r.y.toFixed(0)},${r.w.toFixed(0)},${r.h.toFixed(0)})`;
}
