...
const HTML = path.join(REPO, 'Assets', 'UI', 'leaderboard.html');
const VIEWPORT_W = parseInt(process.argv[2] || '1434', 10);
const VIEWPORT_H = parseInt(process.argv[3] || '781', 10);
            ...
            function walk(el, depth) {
                ...
                if (!isWrapper) {
                    ...
                    if (cs.display !== 'none') {
                        ...
                        out.push({ i: out.length, depth, tag: tag.toLowerCase(), id: el.id || '', cls: (typeof el.className === 'string' ? el.className : '') || '', x: Math.round(r.x * 100) / 100, y: Math.round(r.y * 100) / 100, w: Math.round(r.width * 100) / 100, h: Math.round(r.height * 100) / 100, display: cs.display, position: cs.position });
                    }
                }
