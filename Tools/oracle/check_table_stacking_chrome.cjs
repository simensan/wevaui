...
(async () => {
  ...
  try {
    ...
    const rows = [];
    for (const parentZ of ['auto','0','2','-1']) for (const childZ of ['3','-1']) for (const wrapper of [false,true]) for (const cellBackground of ['transparent','cyan']) for (const backWidth of [20,80]) {
      const name = `parent-${parentZ}-child-${childZ}-wrapper-${wrapper}-background-${cellBackground}-back-${backWidth}`;
