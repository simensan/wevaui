...
(`libweva`, ABI minor 44 at this writing) drives two hosts; the Unity host's
standing is first, the Godot host's -- the longer record -- follows.

## Unity host readiness (2026-09-15)
...
proximity, malformed preludes and boundary dependencies, with 41 Chrome checks
passing on Windows and Linux. GCC, sanitizers, all 334 layout comparisons,
both native plugins and the affected Godot scenes pass again; full Unity
EditMode passes 236 with 2 environment-gated inconclusive. The rendered scope
owner change was inspected. Nested CSS now preserves parent selectors,
declaration order, pseudo-element behavior and component boundaries; all 46
...
were inspected. The collector preserves their images with rewritten URLs
and collision-safe asset names; missing local images fail collection. All
47 sample layouts pass and every sample asset loads. Missing assets now fail
the full gate instead of being waived for the retired Sprite demo.

The repaired Unity layout-oracle runner passes all 47 sample pages against
the standalone core using identical synthetic metrics and frozen animations.
It now uses this checkout, supplies the manifest environment and rejects stale,
failed or inconclusive Unity results. Eight tooling regressions cover the process
handoff, preserved evidence, paths containing shell characters and static inputs.
This verifies host translation; the 334 Chrome captures remain the CSS reference.

The visual-review runner now captures all 47 current samples in Unity and fresh
Chrome, replacing the deleted C# golden inputs. Its report links the PNG pairs
without grading them. The 9-slice and match-3 endgame pairs were inspected;
...
The benchmark runners now reject failed processes, absent metrics, empty corpora
and invalid iteration counts, preserving failed-run logs. Both ran all 47 samples
with their repaired default build paths. The standalone renderer wrappers also
use the current paths; all 12 interactive backend comparisons pass, and a custom
corpus runs through both the backend comparison and the Chrome color survey.
