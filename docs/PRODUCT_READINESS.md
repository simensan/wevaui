...
| Fonts and text | The package's Inter with real bold/italic, `@font-face` `url()`/`local()`, `RegisterFontFamily`. The package's own shaper over the font's OpenType tables (`UnityFontBackend.Shaping.cs`): visual order and mirrored brackets for right-to-left runs, Arabic joining forms and ligatures, marks on their base, pinned by `UnityFontBackendTests` against Segoe UI and rendered against Chrome by eye (`.utmp/showcase/rtl`). Not run: glyph- and class-based contextual lookups, Indic reordering, cursive attachment; a `Font` asset gets no substitutions (no bytes); no colour emoji. These are gaps, not open questions. |
