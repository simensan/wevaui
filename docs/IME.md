...
The source checkout's `Tools/godot-ime-repro/README.md` includes the
...
```powershell
node Tools/oracle/check_ime_chrome.cjs "C:/Program Files/Google/Chrome/Application/chrome.exe"
```
...
```sh
python3 hosts/godot/check_ime_linux.py --godot /path/to/godot --artifacts /path/to/logs
```
...
`Tools/godot-ime-repro` and add `--native-line-edit`. Optional
