#!/usr/bin/env python3
"""Measure range-selector styling on settings forms; no timing gate."""
import argparse
import json
import os
...
def main():
    ...
    parser.add_argument("--selector-family", choices=["range", "validity"], default="range")
    ...
    if args.selector_family == 'validity':
        fixture = project / 'range_performance.gd'
        source = fixture.read_text(encoding='utf-8')
        fixture.write_text(source.replace(':out-of-range', ':invalid').replace(':in-range', ':valid'), encoding='utf-8')
    ...
    for repeat in range(args.runs):
        ...
        for renderer in renderers:
            ...
            run(label, ['--rendering-method', renderer, '--fixed-fps', '60', '--position', '-10000,-10000',
                '--quit-after', '10000', '--scene', 'res://range_performance.tscn'], dict(env, WEVA_RANGE_PERF_OUT=str(destination)))
            result = json.loads(destination.read_text(encoding='utf-8'))
            if not result['passed'] or len(result['rows']) != 20: raise RuntimeError(label + ' incomplete')
            rows.append(dict(repeat=repeat, renderer=renderer, result=result))
            print(label + ': PASS', flush=True)
    report = dict(selector_family=args.selector_family, metadata=metadata, engine_sha256=build_metadata.sha256(engine),
        fixture_sha256=build_metadata.sha256(project / 'range_performance.gd'),
