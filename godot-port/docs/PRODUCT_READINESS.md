...
A held direction repeats after 400 ms at ten steps a second. The core adds
`weva_element_tag_name` for this. Thirty-one host checks push real joypad
events through a viewport, including the repeat timing; physical controller
acceptance remains open. [Gamepad evidence](verification/gamepad-navigation.json).
