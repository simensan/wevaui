...
```sh
python3 godot-port/tools/godot-text-shaping-repro/check.py \
...
`godot-port/check.sh` now runs this suite as **Godot engine text safety**.
Use `bash godot-port/check.sh --release` to also reject skipped gates. This
...
```sh
python3 -m unittest discover -s godot-port/tools/godot-text-shaping-repro -p test_check.py
```

To run one case directly:

```sh
godot --headless --path godot-port/tools/godot-text-shaping-repro \
...
```sh
git apply /path/to/unityui/godot-port/tools/godot-text-shaping-repro/godot-script-iterator.patch
...
WEVA_TEXT_SHAPING_STRESS=1 /path/to/patched-godot --headless \
  --path godot-port/hosts/godot/project text_autoscroll_tests.tscn
```
