...
(async()=>{
 ...
 try {
  ...
  for(const collapse of ['collapse','separate'])for(const side of ['top','bottom'])for(const padding of [0,20]) {
   ...
   const name=[collapse,side,padding].join('-');await page.screenshot({path:process.argv[2]+'.'+name+'.png',omitBackground:true});rows.push({name,html,css,bounds});
  }
