...
func run(scene: Control) -> void:
	...
	if "gamepad_wake" in ui:
		...
		pad(JOY_BUTTON_A)
		await settle()
		check(ui.is_keyboard_open() and ui.get_focused_id() == "player-name", "pad accept on the name field opens the on-screen keyboard")
		await capture("pad-keyboard")
		pad(JOY_BUTTON_A)
		await settle()
		check(game.state.model.Player.Name == "Morganq", "a keyboard key types into the bound name field")
		pad(JOY_BUTTON_B)
		await settle()
		check(not ui.is_keyboard_open() and game.settings_open and ui.get_focused_id() == "player-name", "cancel closes the keyboard, not the settings")
