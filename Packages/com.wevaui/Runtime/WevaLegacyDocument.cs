...
namespace Weva {
    ...
    /// </summary>
    [AddComponentMenu("Weva/Legacy UI Document (frozen C# engine)")]
    ...
    [ExecuteAlways]
    public sealed class WevaLegacyDocument : MonoBehaviour, IUIPaintSource, IRenderViewportAwarePaintSource {
        ...
        void EnsureInputController() {
            ...
            try {
                ...
                Debug.LogWarning($"WevaLegacyDocument on '{name}' could not attach UnityInputController: {ex.Message}", this);
            }
        }
        ...
        void BuildPipeline() {
            if (documentAsset == null) {
                Debug.LogWarning($"WevaLegacyDocument on '{name}' has no document asset; remaining idle.", this);
                return;
            }
            ...
            try {
                ...
                Debug.LogError($"WevaLegacyDocument on '{name}' failed to build: {ex.Message}", this);
                state = null;
            }
        ...
        void LogBindingWarnings(BindingSet bindings) {
            ...
            for (int i = 0; i < warnings.Count; i++) {
                Debug.LogWarning($"WevaLegacyDocument on '{name}': {warnings[i]}", this);
            }
        }
        ...
        void AttachHotReload() {
            try {
                ...
                Debug.LogWarning($"WevaLegacyDocument on '{name}' could not enable hot reload: {ex.Message}", this);
            }
            ...
            try {
                ...
                Debug.LogWarning($"WevaLegacyDocument on '{name}' could not enable HTML hot reload: {ex.Message}", this);
            }
        }
