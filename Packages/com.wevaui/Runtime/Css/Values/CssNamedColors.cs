...
namespace Weva.Css.Values {
    internal static class CssNamedColors {
        // OrdinalIgnoreCase lookup avoids the per-call ToLowerInvariant
        // allocation that the original implementation paid on every named-
        // color resolution. Hot enough on shadow + background + border
        // resolvers that the saving compounds across a paint frame.
        static readonly Dictionary<string, (byte r, byte g, byte b, float a)> table = Build();

        public static bool TryGet(string name, out byte r, out byte g, out byte b, out float a) {
            if (name == null) {
                r = 0; g = 0; b = 0; a = 1;
                return false;
            }
            if (table.TryGetValue(name, out var v)) {
        ...
        static Dictionary<string, (byte, byte, byte, float)> Build() {
            var t = new Dictionary<string, (byte, byte, byte, float)>(StringComparer.OrdinalIgnoreCase);
            void Add(string name, byte r, byte g, byte b) => t[name] = (r, g, b, 1f);
            ...
            t["transparent"] = (0, 0, 0, 0f);
            // CSS Color L4 §10 system colors. Light-mode defaults; no
            // color-scheme / OS-palette integration yet.
            Add("Canvas", 255, 255, 255);
            Add("CanvasText", 0, 0, 0);
            Add("LinkText", 0, 0, 238);
            Add("VisitedText", 85, 26, 139);
            Add("ActiveText", 255, 0, 0);
            Add("ButtonText", 0, 0, 0);
            Add("ButtonFace", 221, 221, 221);
            Add("ButtonBorder", 0, 0, 0);
            Add("Field", 255, 255, 255);
            Add("FieldText", 0, 0, 0);
            Add("Highlight", 180, 213, 254);
            Add("HighlightText", 0, 0, 0);
            Add("SelectedItem", 180, 213, 254);
            Add("SelectedItemText", 0, 0, 0);
            Add("Mark", 255, 255, 0);
            Add("MarkText", 0, 0, 0);
            Add("GrayText", 128, 128, 128);
            Add("AccentColor", 0, 102, 204);
            Add("AccentColorText", 255, 255, 255);
            return t;
        }
    }
}
