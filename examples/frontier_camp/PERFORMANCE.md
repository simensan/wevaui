...
[textserver-subpixel.json](../../docs/verification/textserver-subpixel.json).
Build221 added the on-screen keyboard for controller text entry
([qualification221.json](../../docs/verification/qualification221.json)) on
top of build220's stock-engine shaping workaround, `@font-face` with real
bold/italic files, gamepad navigation and `WevaView` live reload. Build220's
record follows. Its qualification (core, sanitizers,
...
`run_frontier_perf.py --budget examples/frontier_camp/tests/performance_budget_desktop.json`
...
```powershell
python hosts/godot/frontier_perf_budget.py --report path/to/summary.json --budget examples/frontier_camp/tests/performance_budget_desktop.json --output path/to/budget-result.json
python -m unittest discover -s hosts/godot -p test_frontier_perf_budget.py -v
```
...
python hosts/godot/run_frontier_perf.py --godot /path/to/godot \
...
python hosts/godot/run_frontier_lifecycle.py \

  --executable /new/load-1080p/export/FrontierCamp.exe \

  --project examples/frontier_camp --output /new/lifecycle \
